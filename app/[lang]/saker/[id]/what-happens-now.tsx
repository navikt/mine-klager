  ...
  return (
    <VStack as="section" data-element="next-event">
      ...
      >
        ...
        <RelevantDocuments relevantDocuments={lastEvent.relevantDocuments} lang={lang} />
      </Box>
    </VStack>
  );
};

const KlageMottattKlageinstans = (lang: Language) => {
  if (lang === Language.NB) {
    return (
      <>
        <BodyShort spacing>
...
const AnkeMottattKlageinstans = (lang: Language) => {
  if (lang === Language.NB) {
    return (
      <>
...
const NextDescription = ({ type, lang }: NextDescriptionProps) => {
  switch (type) {
    case EventType.KLAGE_MOTTATT_KLAGEINSTANS:
      return KlageMottattKlageinstans(lang);
    case EventType.ANKE_MOTTATT_KLAGEINSTANS:
      return AnkeMottattKlageinstans(lang);
    default:
      return (
        <>
...
const NEXT_DESCRIPTION: Record<SimpleEvent, Record<Language, string[]>> = {
